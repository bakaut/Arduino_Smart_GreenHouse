...
void loop() {
 ...
 // }
  ...
  int days_delta,cur_day,cur_mon,cur_h,cur_m,delta_min;
  
  if (int(makeTime(tm)) > 1627839741) {
    tm.Year = CalendarYrToTm(2018);
    days_delta = (millis()/1000)%86400;
    cur_day = 21 + days_delta;
    if ( cur_day > 31) {
      cur_day = cur_day - 31;
      cur_mon = 7 + 1;
      }
    cur_h = (millis()/1000)%3600;
    if ((cur_h +16) < 24) {
      cur_h = cur_h +16;
      }
    else {
